...
void receive_data(struct connection *conn)
{
	...
	if (recv_len <= 0) {
		...
		dlog(LOG_INFO, "%s, peer: %s\n", recv_len == 0 ? "Receiving connection closed" : "Receiving communication error", addressBuffer);
		conn->state = STATE_CONNECTION_CLOSED;
		return;
	}
...
ssize_t connection_send_data(struct connection *conn)
{
	...
	while (conn->send_len > 0) {
		...
		if (send_len <= 0) {
			...
			dlog(LOG_INFO, "%s, peer: %s\n", send_len == 0 ? "Connection was closed while sending data" : "Communication error while sending data", addressBuffer);
			conn->state = STATE_CONNECTION_CLOSED;
			return -1;
		}
...
void connection_send_dynamic_data(struct connection *conn)
{
	...
	if (send_len <= 0) {
		/* error in communication ( <0 ) or connection closed (0)*/
		dlog(LOG_INFO, "%s, peer: %s\n", send_len == 0 ? "Dynamic send connection closed" : "Dynamic send communication error", addressBuffer);
		return;
	}
